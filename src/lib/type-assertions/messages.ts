export namespace Txt {
    ...
    export type Msg = {
        ...
        // does not subtype:
        not_assignable_to: `${s[">"]} ${w["does"]} ${w["not"]} ${w["subtype"]} ${s["<"]}`
        equals: `${s[">"]} ${w["does"]} ${w["equal"]} ${s["<"]}`
        are_both_any: `${s[">"]} 𝗮𝗿𝗲 ${w["are_both"]} ${w["any"]}`

        assignable_to: `${s[">"]} ${w["does"]} ${w["subtype"]} ${s["<"]}`
        assignable_from: `${s[">"]} ${w["does"]} ${w["supertype"]} ${s["<"]}`
        resembles: `${s[">"]} ${w["does"]} ${w["resemble"]} ${s["<"]}`

        // lower case: not a supertype of
        not_assignable_from: `${s[">"]} ${w["does"]} ${w["not"]} ${w["supertype"]} ${s["<"]}`
